...
class Emitter:
    ...
    def emit_event(
        ...
        tid = tid_or_kernel(msg.event)
        args = dict(msg.event.payload_field)
        track_uuid = None
        ...
        if name == "thread_create":
            self.threads[tid] = {'pid': args['pid'], 'name': args['name'], 'prio':
                                 args['prio'], 'ts': self.event_us(msg)}
            return  # meta event
        ...
        if type(name) is SyntheticEvents:
            ...
            match name:
                ...
                case SyntheticEvents.LOCKED:
                    ...
                    flow_id = int(msg.event.payload_field["lid"])
            ...
            assert phase != TrackEvent.Type.TYPE_SLICE_BEGIN \
                and phase != TrackEvent.Type.TYPE_SLICE_END

            event_name = name

            if name == "lock_name":
                lock_id = int(msg.event.payload_field["lid"])
                lock_name = str(msg.event.payload_field["name"])
            ...
            if name == "thread_priority":
                self.print_trace_packets(
                    [self.emit_prio_change(tid,
                                           args['priority'],
                                           self.event_us(msg))])
                return  # meta event

            if name == "thread_end":
                self.end_ongoing_events(tid, self.event_us(msg))
        ...
        packet = perfetto_trace_pb2.TracePacket()
        packet.timestamp = self.event_us(msg) + (1 if shift_by_ns else 0)
